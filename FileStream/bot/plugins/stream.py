...
async def private_receive_handler(bot: Client, message: Message):
    ...
    if message.from_user.id == Telegram.OWNER_ID:
        pass
    elif file_name1 not in fn3:
        await message.reply_text("**Sorry, This File Is Not Associated With @MEDIA_LINKS.\n\nStreaming Link For This File Can Only Be Generated By Admins Of @OLD_CARTOONSTAMIL.**")
        await bot.send_message(chat_id=Telegram.ULOG_CHANNEL,text=f"**User [{message.from_user.first_name}](tg://user?id={message.from_user.id}) Sent Files which Is Not Available In DB\n\nᴜsᴇʀ ɪᴅ :** `{str(message.from_user.id)}`")
        return
    try:
